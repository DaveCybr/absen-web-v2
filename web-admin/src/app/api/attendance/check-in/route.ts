...
const MAX_PHOTO_BASE64_LENGTH = 7 * 1024 * 1024; // ~5MB
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Karyawan hanya bisa check-in untuk dirinya sendiri
    const {
      data: { user },
      error: authError,
    ...
    // Cek sudah check-in hari ini (WIB)
    ...
    // ✅ FIX [C1]: Gunakan face_image_url (dari storage) bukan face_token
    // face_token Face++ expire 24 jam, face_image_url permanent di storage
